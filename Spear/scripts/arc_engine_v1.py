...
## = Line I wrote. The rest is from DaFluffyPotato
...
class physics_obj(object):
    ...
    def move(self,movement,platforms,ramps=[]): 
        self.x += movement[0]
        if movement[0] > 0 and (self.x % 1) > 0: ##
            self.x += (1 - (self.x % 1)) ##
...
class entity(object):
    ...
    def move(self,momentum,platforms,ramps=[]):
        ...
        self.y = self.obj.y
        if self.held_item:
            self.held_item.x = self.x+self.size_x/2
            self.held_item.y = self.y+self.size_y/2+3
        return collisions
